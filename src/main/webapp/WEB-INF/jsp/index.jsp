...
<style type="text/css">
	...
	************************************************************************************************************/
	...
	<c:forEach items="${size}" var="sizeValue">
	
	boxSizeArray.push("${sizeValue}");
	...
	*/
	...
	function removeExtraLi(){
		
		var dragDropTopContainer2 = document.getElementById('dhtmlgoodies_dragDropContainer');
		var listItems2 = dragDropTopContainer2.getElementsByTagName('LI');
		for(var no=0;no<listItems2.length;no++){
			
			
			
			if(listItems2[no].innerHTML.trim() == ""){
			
				listItems2[no].parentNode.removeChild(listItems2[no]);
			}
		}
...
<form:form modelAttribute="team" action="createTeam" method="post">
<form:input path="teamName" placeholder="Team Name"/>
<form:input path="teamSize" placeholder="Max Team size"/>
<form:button type="submit">Create Team </form:button>
</form:form>

</div>

<div style="margin:9px">
<form:form modelAttribute="student" action="createStudent" method="post">
<form:input path="name" placeholder="Student Name"/>
...
<c:if test= "${teamindex.count == 1}">
	<div id="dhtmlgoodies_listOfItems">
		<div>
			<p>Available students</p>
		<ul id="${team.id}">
		<c:forEach items="${team.students}" var="students">
			<li id="${students.id}">${students.name}<li>
			</c:forEach>
		</ul>
		</div>
	</div>
...
<div id="dragDropIndicator"><img src="${pageContext.request.contextPath}/resources/images/insert.gif"></div>
